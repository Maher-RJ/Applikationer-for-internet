...
class DbAccess {
	...
	public function clean($string){
		...
		$string = mysqli_escape_string($this->connect(), stripslashes($string));
		return $string;
	...
	public function insertComment(Comment $data)
	{
		...
		$comment = $this->clean($data->getComment());
		$recipe = $data->getDish();
		...
		if ($sql = mysqli_query($connect, $query))
		{
			$data = array('id' => mysqli_insert_id($connect), 'username' => $username, 'comment' => $comment);
